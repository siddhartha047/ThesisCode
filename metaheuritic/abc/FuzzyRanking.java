...
public class FuzzyRanking {
   ...
   */
  private static final Comparator fuzzydominance = new FuzzyDominanceComparator();
  ...
  public int NumberofFronts=8;
  
  public FuzzyRanking(SolutionSet solutionSet,int partition) {        
    solutionSet_ = solutionSet ;

    NumberofFronts=partition;
    ...
    */
    ...
    for (int p = 0; p < (solutionSet_.size()-1); p++) {
      ...
      for (int q = p+1; q < solutionSet_.size(); q++) {
        flagDominate =constraint_.compare(solutionSet.get(p),solutionSet.get(q));
        if (flagDominate == 0) {
          flagDominate =fuzzydominance.compare(solutionSet.get(p),solutionSet.get(q));
        }
        if (flagDominate == -1)
        {
        //  System.out.println(p+"\tdominate\t"+q);
          iDominate[p].add(new Integer(q));
          dominateMe[q]++;
        }
        else if (flagDominate == 1)
        {
          //System.out.println(p+"\tdominated by\t"+q);
        ...
        if(dominateMe[p]>maxNumberOfDomination)maxNumberOfDomination=dominateMe[p];
        if(dominateMe[q]>maxNumberOfDomination)maxNumberOfDomination=dominateMe[q];
      }
    }
    
    maxNumberOfDomination+=1;
    
    for (int p = 0; p < solutionSet_.size(); p++) {
    
    	for(int i=0;i<maxNumberOfDomination;i++){
    		if (dominateMe[p] == i) {
    	        front[i].add(new Integer(p));
    	        break;
    	    }
    	}      
    }    
            
    ranking_ = new SolutionSet[maxNumberOfDomination];
    
    System.out.println("max domination-> "+maxNumberOfDomination);
    
    Iterator<Integer> it1;
    
    int rank=0;
    
    int minFrontSize=solutionSet_.size()/NumberofFronts;
    		
    for (int j = 0; j < maxNumberOfDomination; j++) {
      if(front[j].size()==0)continue;
      
      int size=0;
      int k=0;
      
      for(k=j;k<maxNumberOfDomination;k++){
    	  size+=front[k].size();
    	  if(size>minFrontSize)break;
      }
      
      ranking_[rank]=new SolutionSet(size);      
      
      for(int l=j;l<=k;l++){
    	  it1 = front[l].iterator();
	      while (it1.hasNext()) {
	                ranking_[rank].add(solutionSet.get(( it1.next()).intValue()));       
	      }
      }
    ...
    System.out.println("fronts->\t"+rank);
  
    int total=0;
    for(int inc=0;inc<rank;inc++){
    	total+=ranking_[inc].size();
    	System.out.println(ranking_[inc].size());
    }
    
    System.out.println("Total->"+total);
     
    if(total!=solutionSet.size()){
    	System.out.println("Couldn't classify all");
    	System.exit(0);
    }
