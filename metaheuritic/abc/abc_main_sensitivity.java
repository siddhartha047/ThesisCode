...
public class abc_main_sensitivity {
	...
	public static void main(String[] args) throws JMException,
		...
		int[] dimArr = { 13};
		int[] wfgno = {2,3,4,5,6,7,8,9};
		double[] seedar = {.1,.3,.5, .9};
		int[] param = {0,2, 4, 6, 8, 10,12};
		//int[] param = {0};

		// double
		// []seedar={0.05,0.1,0.15,0.2,0.25,0.3,0.35,0.4,0.45,0.5,0.55,0.6,0.65,0.7,0.75,0.8,0.85,0.9,0.95,1};
		// double []seedar={0.05,0.15,0.2,0.25,0.3,0.35,0.4,0.45};
		// double []seedar={0.55,0.6,0.65,0.7,0.75,0.8,0.85,0.95,1};

		// int []dimArr={4};

		for (int parameter = 0; parameter < param.length; parameter++) {
			...
			for (int dimno = 0; dimno < dimArr.length; dimno++) {
				...
				for (int wfgProblemNo = 0; wfgProblemNo < wfgno.length; wfgProblemNo++) {
					...
					QualityIndicator indicators; // Object to get quality
													// indicators
					...
					if (args.length == 1) {
						...
						problem = (new ProblemFactory()).getProblem(args[0],
								params);
					} // if
						...
						// DTLZ settings
						/*
						 * int dtlzwhat=2; int number_of_objective=5; int
						 * number_of_decvars=number_of_objective+9; Object []
						 * params
						 * ={"Real",number_of_decvars,number_of_objective};
						 * problem = (new
						 * ProblemFactory()).getProblem("DTLZ"+dtlzwhat,params);
						 * indicators = new QualityIndicator(problem,
						 * read_settings
						 * .pf_path+"/DTLZ"+dtlzwhat+"_"+String.valueOf
						 * (number_of_objective)+"D.pf") ;
						 * System.out.println(read_settings
						 * .pf_path+"/DTLZ"+dtlzwhat
						 * +"_"+String.valueOf(number_of_objective)+"D.pf");
						 */
					...
					int parameterValue=param[parameter];
					
					algorithm = new SidNaheed(problem,parameterValue);
					...
					for (int i = 0; i < seedar.length; i++) {
						...
						String paramFolder=read_settings.generation_path
								+ "\\"+String.valueOf(parameterValue);
						
						folderCreation(paramFolder);
						
						String seedFolder = read_settings.generation_path
								+ "\\"+String.valueOf(parameterValue)+"\\" + String.valueOf(seedar[i]);
						
						folderCreation(seedFolder);
						...
						String resultTxt = seedFolder + "\\" + resultFile
								+ ".txt";
						...
						if (indicators != null) {
							 ...
							 * output
							 * .write("Spread "+String.valueOf(Spread)+"\n");
							 * output
							 ...
							 * logger_.info("True Hypervolume: "+
							 * indicators.getTrueParetoFrontHypervolume()); //
							 * logger_.info("Hypervolume: "+
							 * indicators.getHypervolume(population)); //
							 * logger_.info("Hypervolume Ratio    : "+
							 * indicators.getHypervolume_Ratio(population)+
							 * " *****************");
							 * logger_.info("GD         : " +
							 ...
							 * logger_.info("Spread     : " +
							 * indicators.getSpread
							 * (population)+"##########################");
							 * logger_.info("Epsilon    : " +
							 * indicators.getEpsilon(population)); logger_.info(
							 * "----------------------------------------------"
							 * );
							 */
							// int evaluations =
							// ((Integer)algorithm.getOutputParameter("evaluations")).intValue();
							// logger_.info("Speed      : " + evaluations/pop +
							// " gen") ;
						} // if
