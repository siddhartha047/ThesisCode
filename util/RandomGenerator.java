...
public class RandomGenerator implements IRandomGenerator{
  ...
  /* 
   * called by PseudoRandom.setseed()
   * author:nahid 
   */
  ...
  double randomperc(){
    ...
    if(jrand>=55){
      ...
      //System.out.println("oldrand[jrand]:"+oldrand[jrand]);
  ...
  synchronized public int rndInt (int low, int high){
     // System.out.println("seed:"+seed+"low:"+low+"high:"+high);
    int res;
    if (low >= high){
      res = low;
    } else {
     // res = low + (int)(randomperc()*(high-low+1));
         double p = randomperc()*(high-low+1);
